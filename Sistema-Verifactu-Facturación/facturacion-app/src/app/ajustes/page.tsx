...
import { Save, Building2, CreditCard, FileText, RotateCcw, Palette, ShieldCheck, Check, AlertTriangle, Loader2 } from 'lucide-react';
...
import { PAYMENT_METHODS, PROVINCES, BUSINESS_SECTORS, ACCENT_THEMES } from '@/lib/constants';
    ...
    <div className="animate-fade-in" style={{ maxWidth: 880 }}>
      ...
      <div className="settings-section">
        ...
        <div className="form-group">
          ...
          />
          <span className="form-hint">Es una clave pública: puede guardarse aquí sin riesgo.</span>
        </div>

        <div className="callout callout-warning" style={{ marginTop: 'var(--space-4)' }}>
          <AlertTriangle size={16} />
          <div>
            <strong>Las claves secretas no se guardan en la aplicación.</strong>
            <p>
              La <code>Secret Key</code> y el <code>Webhook Secret</code> permiten mover dinero de
              tu cuenta. Guardarlas aquí las dejaría en una tabla que el navegador puede leer, así
              que se configuran como variables de entorno del servidor:
            </p>
            <pre className="callout-code">{`STRIPE_SECRET_KEY=sk_live_...
STRIPE_WEBHOOK_SECRET=whsec_...`}</pre>
            <p>
              Añádelas en tu proveedor de hosting (o en <code>.env.local</code> en desarrollo) y
              apunta el webhook de Stripe a <code>/api/stripe/webhook</code>.
            </p>
          </div>
        </div>
      </div>
