...
const APP_KEY_VAR = 'APP_SECRETS_ENCRYPTION_KEY';
const CERT_KEY_VAR = 'CERTIFICATE_ENCRYPTION_KEY';

function getEncryptionKey(): Buffer {
  const raw = process.env[APP_KEY_VAR] || process.env[CERT_KEY_VAR];
  if (!raw) {
    throw new Error(
      `Falta la variable de entorno ${APP_KEY_VAR} o ${CERT_KEY_VAR}. Sin ella el servidor no puede cifrar secretos y, por seguridad, se niega a guardarlos en texto plano.`
    );
  }
  ...
  if (key.length !== 32) {
    throw new Error(
      `La clave debe decodificar a 32 bytes en base64 (clave AES-256). Longitud actual: ${key.length} bytes.`
    );
  }
  return key;
}
...
/**
 * Cifra un texto en plano (ej. claves de Stripe).
 * Retorna un string en base64 (IV + AuthTag + Ciphertext) listo para guardar en BD.
 */
...
/**
 * Descifra un texto previamente cifrado con encryptString (base64).
 */
