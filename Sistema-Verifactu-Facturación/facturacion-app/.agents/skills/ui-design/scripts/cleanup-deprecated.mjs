#!/usr/bin/env node
/**
 * Cleans up deprecated Uizze skill files, symlinks, and
 ...
 *   node scripts/cleanup-deprecated.mjs
 *
 * What it does:
 ...
 *      "uizze" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to uizze
  'teach-uizze',   // folded into /uizze init
  // v2.1 merges
  'arrange',            // renamed to layout
  'normalize',          // merged into polish
  'onboard',            // merged into harden
  'extract',            // merged into /uizze extract
  // v3.0 consolidation: all standalone skills -> /uizze sub-commands
  'adapt',
  'animate',
  'audit',
  'bolder',
...
// "uizze" in their v2.x source. Used as a last-resort match
...
/**
 * Check whether a skill directory belongs to Uizze. Three layered
 * signals, in order of reliability:
 *   1. Lock source equals "pbakaus/uizze" (authoritative).
 *   2. SKILL.md body contains the word "uizze".
 ...
 */
export function isUizzeSkill(skillDir, { skillName, lock } = {}) {
  // 1. Authoritative: the lock file claims this skill is ours.
  if (skillName && lock?.skills?.[skillName]?.source === 'pbakaus/uizze') {
    return true;
  }
  ...
  if (/uizze/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions uizze.
    ...
    for (const name of targets) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        const isMatch = targetAlive
          ? isUizzeSkill(skillPath, { skillName: name, lock })
          : true;
        if (isMatch) {
      ...
      // Regular directory -- verify it belongs to uizze
      if (isUizzeSkill(skillPath, { skillName: name, lock })) {
 ...
 * Only removes entries whose source is "pbakaus/uizze".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to uizze
    if (entry.source === 'pbakaus/uizze') {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(new URL(import.meta.url).pathname)) {
  ...
  if (result.deletedPaths.length === 0 && result.removedLockEntries.length === 0) {
    console.log('No deprecated Uizze skills found. Nothing to clean up.');
  } else {
